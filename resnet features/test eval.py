...
class COCOTestDataset(Dataset):
    ...
    def __init__(self, transform=None):
        ...
        self.h = h5py.File(os.path.join('caption data', 'TEST_IMAGES_coco' + '.hdf5'), 'r')
        self.imgs = self.h['images']

        with open(os.path.join('caption data', 'TEST_names_coco' + '.json'), 'r') as j:
            self.image_names = json.load(j)
            
        with open(os.path.join('caption data', 'TEST_CAPUTIL_coco' + '.json'), 'r') as j:
...
recent_cider, recent_bleu4 = evaluate(val_loader = test_loader, 
                                      encoder = encoder, 
                                      decoder = decoder,
                                      beam_size = 3, 
                                      embed_dim = emb_dim, 
