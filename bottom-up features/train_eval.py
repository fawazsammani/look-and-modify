...
class COCOTrainDataset(Dataset):
    ...
    def __init__(self):
        ...
        with open(os.path.join('caption data', 'TRAIN_GENOME_DETS.json'), 'r') as j:
            self.objdet = json.load(j)
        
        with open('caption data/TRAIN_IMAGE_NAMES.json', 'r') as j:
            self.names = json.load(j)
            
        with open(os.path.join('caption data','TRAIN_CAPUTIL_coco_topdown_bottomup_features.json'), 'r') as j:
    ...
    def __getitem__(self, i):
        ...
        img_name = self.names[i // self.cpi][0]
...
def evaluate(val_loader, decoder, beam_size, embed_dim, epoch, vocab_size):
