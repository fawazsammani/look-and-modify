...
class COCOValidationDataset(Dataset):
    ...
    def __init__(self):
        ...
        with open(os.path.join('caption data',  'VAL_GENOME_DETS.json'), 'r') as j:
            self.objdet = json.load(j)
        
        with open('caption data/VAL_IMAGE_NAMES.json', 'r') as j:
            self.names = json.load(j)
            
        with open(os.path.join('caption data', 'VAL_CAPUTIL_coco_topdown_bottomup_features' + '.json'), 'r') as j:
...
class DecoderWithAttention(nn.Module):
    ...
    def forward(self, attributes, sentence_embed, image_features, encoded_captions, caption_lengths):
        ...
        image_features of shape: (batch_size, 36, 2048)
        ...
        # Flatten image
        image_features_mean = image_features.mean(1).to(device)  # (batch_size, 2048)
...
checkpoint = torch.load('BEST_checkpoint_29.pth.tar')
...
def caption_beamsearch(img, sentence_embed, sentence_attr, decoder, beam_size, vocab_size):
    ...
    seqs_alpha = torch.ones(k, 1, 6, 6).to(device)  # (k, 1, enc_image_size, enc_image_size)
    ...
    while True:
        ...
        alpha = alpha.view(-1, 6, 6)  # (s, enc_image_size, enc_image_size)
...
with open('caption data/VAL_CAPUTIL_coco_topdown_bottomup_features.json', 'r') as k:
...
def visualize_att(image_path, seq, alphas, rev_word_map, smooth=True):
    ...
    image = image.resize([6 * 24, 6 * 24], Image.LANCZOS)
...
for (img, sentence_embed, sentence_attr, image_name) in val_loader:
    ...
    seq, alphas = caption_beamsearch(img, sentence_embed, sentence_attr, decoder, beam_size = 3, vocab_size = len(word_map))
