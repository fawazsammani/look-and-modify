...
class COCOTestDataset(Dataset):
    ...
    def __init__(self):
        ...
        with open(os.path.join('caption data',  'TEST_GENOME_DETS.json'), 'r') as j:
            self.objdet = json.load(j)
        
        with open('caption data/TEST_IMAGE_NAMES.json', 'r') as j:
            self.names = json.load(j)
            
        with open(os.path.join('caption data', 'TEST_CAPUTIL_coco_topdown_bottomup_features' + '.json'), 'r') as j:
...
def evaluate(test_loader, decoder, beam_size, embed_dim, epoch, vocab_size):
    
    decoder.eval()
    results = []
    # For each image
    for i, (img, sentence_embed, sentence_attr, image_id) in enumerate(tqdm(test_loader, 
...
checkpoint = 'BEST_checkpoint.pth.tar'  # path to checkpoint, None if none
...
recent_cider, recent_bleu4 = evaluate(test_loader = test_loader, 
                                      decoder = decoder,
                                      beam_size = 3, 
                                      embed_dim = emb_dim, 
                                      epoch = 0, 
...
print("CiDEr: {}".format(recent_cider))
print("BLEu-4: {}".format(recent_bleu4))
